...
class Version(SQLObject):
    def restore(self):
        ...
        for _col in self.extraCols:
            del values[_col]
        self.mainClass.get(self.mainID).set(**values)

    def nextVersion(self):
        version = self.select(
            AND(self.q.mainID == self.mainID, self.q.id > self.id),
            orderBy=self.q.id)
        if version.count():
            return version[0]
        else:
    ...
    def getChangedFields(self):
        ...
        columns = self.mainClass.sqlmeta.columns
        fields = []
        for column in columns:
            if column not in ["dateArchived", "id", "mainID"]:
    ...
    def select(cls, clause=None, *args, **kw):
        if not getattr(cls, '_connection', None):
            cls._connection = cls.mainClass._connection
...
class Versioning(object):
    ...
    def __addtoclass__(self, soClass, name):
        ...
        attrs = {'dateArchived': col.DateTimeCol(default=datetime.now),
                 'main': col.ForeignKey(self.soClass.__name__),
                 'mainClass': self.soClass,
    ...
    def __get__(self, obj, type=None):
        ...
        return self.versionClass.select(
            self.versionClass.q.mainID == obj.id, connection=obj._connection)
